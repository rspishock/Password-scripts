...
my @password;
my $password;
my ($pw_number, $ex_number, $current) = 1;
#my $ex_number = 1;
#my $current = 1;
my ($password_length, $user_length, $user_number) = 0;
...
print "How many passwords do you need to generate? ";
...
GENERATE:
if ($user_number > 1) {
	while ($pw_number <= $user_number) {
		while ($password_length <= ($user_length-1)) {
  				$password = $chars[int rand @chars];
  				push @password, $password;
				++$password_length;
		}
		push @temp_pwd, @password;
		++$pw_number;
	}
#Single password will not be printed to terminal	
} else {
	while ($password_length <= ($user_length-1)) {
  		$password = $chars[int rand @chars];
  		push @temp_pwd, $password;
...
if ($export =~ m/^y/i) {
	SEPERATE:
	print "Do you want to write a seperate file for each password? ";
	chomp (my $seperate = <STDIN>);
	if ($seperate =~m/^y/i)	{
		while ($ex_number <= $user_number) {
			open (FH, ">>user_$ex_number.txt") or die $!;
			pop @temp_pwd;
			print FH "Your password is: ", @temp_pwd, "\n";
			close FH;
			++$ex_number
		}
	} elsif ($seperate =~ m/^n/i) {
		...
		#Single password not printed to terminal
		foreach (@temp_pwd) {
			print "Your password is: ", @temp_pwd, "\n";
			pop @temp_pwd;
		}
} else {
	print "Enter either 'y' or 'n' to continue. \n" ;
